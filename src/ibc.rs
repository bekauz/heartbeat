...
use crate::{msg::ExecuteMsg, contract::schedule_cosmos_msg, error::{ContractError, Never}, state::CONNECTION_COUNTS, ack::make_ack_fail};

pub const IBC_VERSION: &str = "heartbeat-1";
...
pub fn do_ibc_packet_receive(
    ...
    let recv_msg: ExecuteMsg = from_binary(&msg.packet.data)?;

    match recv_msg {
        ExecuteMsg::Schedule { msg } => {
            let resp = schedule_cosmos_msg(msg, deps)?;
            Ok(IbcReceiveResponse::new()
                .add_attributes(resp.attributes)
            )
        },
        ExecuteMsg::Beat {  } => Ok(IbcReceiveResponse::new()
            .add_attribute("method", "beat")
        ),
    }
}
